...
impl Server {
    ...
    pub fn get_lobby(&mut self, pid: &str) -> Result<&mut Lobby, Error> {
        if self.lobbies.contains_key(pid) {
            return self.lobbies.get_mut(pid).ok_or_else(|| anyhow!("err lobby not in memory".to_string()));
        } else {
...
pub async fn create_document(&mut self, pid: &str, collection_name: &str, document: Document) -> Result<String, Error> {
    let lobby = self.get_lobby(pid)?;
    let doc_id = lobby.create_document(collection_name, document.clone()).map_err(|_e| anyhow!(""))?;
    
    // broadcast to all subscribers
    self.broadcast_to_lobby(pid, Output::DocumentCreated {
